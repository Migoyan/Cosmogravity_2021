...
function trajectoire() {
    ...
    */
    ...
    racines = CubicSolve(E, 0, Math.pow(a,2)*E, rs*Math.pow(a,2)*E-rs*a*L);
    if(racines[0]["i"]==0){
        arrayracinesreels.push(racines[0]["real"]);
    }	
    if(racines[1]["i"]==0){
        arrayracinesreels.push(racines[1]["real"]);
    }
    if(racines[2]["i"]==0){
        arrayracinesreels.push(racines[2]["real"]);
    }
    //console.log("arrayracinesreels",arrayracinesreels);
    racinemax=rhp;
    for (var iiiii = 0; iiiii < arrayracinesreels.length; iiiii++) {
        if(arrayracinesreels[iiiii]>=racinemax && arrayracinesreels[iiiii]<=rmax * 1.2){
...
function majFondFixe(){
	...
	context.fillText("V\u2080 = "+c.toExponential(8)+" m.s\u207B\u00B9",5,150);
